...
basemap = leaflet(plot_map) %>% 
  ...
  hideGroup(c("2019-COVID (active)", "2019-COVID (new)", "2003-SARS", "2009-H1N1 (swine flu)", "2014-Ebola", "RHINO"))  %>%
...
sars_basemap = leaflet(sars_plot_map) %>% 
  ...
  #######
  addCircleMarkers(data = rhino_cases, lat = ~ latitude, lng = ~ longitude, weight = 1, radius = ~(projected_deaths)^(1/4),
                   fillOpacity = 0.2, color = h1n1_col, group = "RHINO (MAPPING)",
                   label = sprintf("<strong>%s</strong><br/>Birth (confirmed): %g<br/>RHINO deaths (estimated): %g", rhino_cases$region, rhino_cases$deaths, rhino_cases$projected_deaths) %>% lapply(htmltools::HTML),
                   labelOptions = labelOptions(
...
server = function(input, output, session) {
  ...
  observeEvent(input$plot_date, {
    ...
    addPolygons(data = reactive_polygons(), stroke = FALSE, smoothFactor = 0.1, fillOpacity = 0.15, fillColor = ~cv_pal(reactive_db_large()$deathsper100k)) %>%
      ...
      addCircleMarkers(data = rhino_cases, lat = ~ latitude, lng = ~ longitude, weight = 1, radius = ~(projected_deaths)^(1/4), 
                       fillOpacity = 0.2, color = h1n1_col, group = "RHINO (BIRTH)",
                       label = sprintf("<strong>%s</strong><br/>RHINO Birth (confirmed): %g<br/>Birth birth (estimated): %g", rhino_cases$region, rhino_cases$deaths, h1n1_cases$projected_deaths) %>% lapply(htmltools::HTML),
                       labelOptions = labelOptions(
                         style = list("font-weight" = "normal", padding = "3px 8px", "color" = rhino_col),
                         textsize = "15px", direction = "auto")) %>%
  ...
  observeEvent(input$sars_plot_date, {
    leafletProxy("sars_map") %>% 
      ...
      addCircleMarkers(data = rhino_cases, lat = ~ latitude, lng = ~ longitude, weight = 1, radius = ~(projected_deaths)^(1/4),
                       fillOpacity = 0.2, color = h1n1_col, group = "RHINO (BIRTH)",
                       label = sprintf("<strong>%s</strong><br/>RHINO birth (confirmed): %g<br/>RHINO birth (estimated): %g", rhino_cases$region, rhino_cases$deaths, rhino_cases$projected_deaths) %>% lapply(htmltools::HTML),
                       labelOptions = labelOptions(
                         style = list("font-weight" = "normal", padding = "3px 8px", "color" = h1n1_col),
                         textsize = "15px", direction = "auto")) %>%
